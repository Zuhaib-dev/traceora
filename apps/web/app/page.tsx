...
emitTraceEvent({
  ...
  metadata: { query: 'SELECT * FROM users' }
})`} /> : <div className="rounded-xl border border-border bg-card p-8"><div className="flex items-center gap-3 text-primary"><Sparkles className="size-5" /><span className="font-mono text-xs">traceora.devtools</span></div><h3 className="mt-8 text-2xl">Your app, with context.</h3><p className="mt-3 max-w-md leading-7 text-muted-foreground">Every event is grouped by TraceID, so debugging stops being archaeology and starts being a straight line.</p><a href="#" className="mt-8 inline-flex items-center gap-2 text-sm text-primary">Read the full guide <ArrowRight className="size-4" /></a></div>}</div></div></section>
    ...
    <footer id="footer" className="border-t border-border"><div className="mx-auto flex max-w-7xl flex-col gap-8 px-5 py-10 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between lg:px-8"><div><Logo /><p className="mt-3 text-xs">Zero-config full-stack runtime intelligence.</p></div><div className="flex items-center gap-5 text-xs"><span>Built by <a href="#" className="text-foreground hover:text-primary">Zuhaib Rashid</a></span><a href="https://github.com" aria-label="GitHub" className="hover:text-foreground"><GitBranch className="size-4" /></a><a href="https://x.com" aria-label="X" className="hover:text-foreground"><X className="size-4" /></a><a href="#" className="hover:text-foreground">Portfolio</a></div></div></footer>
  </main>
}
